  ...
  return (
    <>
      ...
      <section className="section review-layout">
        ...
        <article className="card"><div className="card-head"><div><p className="eyebrow">OFFICIAL PLAYER / HONEST FALLBACK</p><h2>{visiblePreview?.label ?? (ru ? "Прослушивание по клику" : "Playback on click")}</h2></div>{visiblePreview && <span className="badge">ONE ACTIVE</span>}</div><PlayerPanel preview={visiblePreview} language={language} onClose={() => setPreview(undefined)} /></article>
        ...
        <article className="card"><div className="card-head"><div><p className="eyebrow">EXACT DESTINATION URL IDs · PROVENANCE SHOWN</p><h2>{ru ? "Кандидаты" : "Candidates"}</h2></div><span className="badge">← →</span></div>
          ...
          {selectedCandidate && selectedTarget && <div className="candidate-detail">
            ...
            <CandidateProvenancePanel candidate={selectedCandidate} language={language} />
            ...
            <div className="target-id"><small>{selectedTarget.provider === "youtube" ? "videoId (required)" : "providerEntityId"}</small><code>{selectedTarget.videoId ?? selectedTarget.providerEntityId ?? "—"}</code></div>
            ...
            <div className="page-actions"><button className="button" type="button" onClick={() => setPreview(candidatePreview)}>{selectedTarget.provider === "youtube" ? selectedCanSeekYoutube ? (ru ? "Слушать с 25%" : "Listen near 25%") : (ru ? "Открыть честный link-out" : "Open honest link-out") : "Sequential link-out"}</button><button className="button primary" type="button" disabled={busy || !selectedTarget.providerEntityId || (selectedTarget.provider === "youtube" && !selectedTarget.videoId)} onClick={() => void decide("select", selectedTarget)}>{ru ? "Выбрать этот точный ID" : "Select this exact ID"}</button></div>
